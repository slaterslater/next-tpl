...
export default function Score({gameString}) {
  // console.log({eventData})
  // const {awayId, homeId, awayEvents, homeEvents} = eventData
  const [gameId, awayId, homeId] = gameString.split("-");
  
  const awayEvents = use(getData(`gameEvents/${gameId}/${awayId}`))
console.log({awayEvents})
  ...
  return (
    <>
      ...
      <tbody>
        {current === awayId && <Stats />}
        {current === homeId && <Stats />}
      </tbody>
      </table> */}
    </>
  )
